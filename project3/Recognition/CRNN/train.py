...
if __name__ == '__main__':
    ...
    if not taskconfig.hp.pretrained_model_path == '':
        #finetune_dict = torch.load(taskconfig.hp.pretrained_model_path)
        #model_dict = model.state_dict()
        #finetune_dict = {k:v for k,v in finetune_.items() if k in model_dict}
        #model_dict.update(finetune_dict)
        model.load_state_dict(torch.load(taskconfig.hp.pretrained_model_path),strict = False)
