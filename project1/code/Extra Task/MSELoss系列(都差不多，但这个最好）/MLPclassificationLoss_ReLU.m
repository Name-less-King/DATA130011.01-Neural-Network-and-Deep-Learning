...
for i = 1:nInstances
    ...
    if nargout > 1
        ...
        if length(nHidden) > 1
            ...
            clear backprop
            for c = 1:nLabels
                backprop(c,:) = err(c)*(ReLU_bp(ip{end}).*outputWeights(:,c)');
            ...
            for h = length(nHidden)-2:-1:1
                backprop = (backprop*hiddenWeights{h+1}').*ReLU_bp(ip{h+1});
                gHidden{h} = gHidden{h} + fp{h}'*backprop;
            end

            % Input Weights
            backprop = (backprop*hiddenWeights{1}').*ReLU_bp(ip{1});
            gInput = gInput + X(i,:)'*backprop;
        else
           % Input Weights
            for c = 1:nLabels
                gInput = gInput + err(c)*X(i,:)'*(ReLU_bp(ip{end}).*outputWeights(:,c)');
            end
        end
